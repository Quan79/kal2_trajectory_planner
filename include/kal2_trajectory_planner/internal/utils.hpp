...
// 计算多项式值
inline double polynomialValue(const Eigen::VectorXd& coeffs, double x) {
...
// 计算点到多项式曲线的偏离值
inline double deviationFromPolynomial(const Eigen::VectorXd& coeffs, double x, double y) {
    double curveY = polynomialValue(coeffs, x);
    return std::abs(y - curveY);
}

// 拟合多项式并剔除最大偏离值
inline Eigen::VectorXd fitPolynomialWithMaxDeviation(const std::vector<Point>& points, int degree, double maxDeviation) {
    ...
    for (int i = 0; i < numPoints; ++i) {
        double xi = points[i].x;
        double yi = points[i].y;
        double deviation = deviationFromPolynomial(coeffs, xi, yi);
        if (deviation > maxDev) {
            maxDev = deviation;
        }
    }

    // 如果最大偏离值大于阈值，则剔除最大偏离值对应的点，重新进行多项式拟合
    if (maxDev > maxDeviation) {
        ...
        return fitPolynomialWithMaxDeviation(remainingPoints, degree, maxDeviation);
    }
...
inline void startandtraget(const std::vector<Point>& linkcones, const std::vector<Point>& rightcones,  Point& startpoint, Point& targetpoint) {
    ...
    if (i < 2 && j < 2) {
        std::cout << "cones数量太少  können nicht startpoint rechnen "<<std::endl<<std::endl;
        return;
